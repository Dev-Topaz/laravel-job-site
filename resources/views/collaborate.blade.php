...
<header class="bg-white">
  ...
  <script>
var cropper,filesValue,$modal=$("#modal"),image=document.getElementById("image");function onClickCustom(){if("custom"==$("select#price").val()){console.log("clicked"),$("select#price").remove();var e=$("<input name='price' type='text' id='price' value='{{ old('price') }}' class='block w-full pl-3 pr-12 sm:text-sm border-gray-300 rounded-md @error('price') is-invalid @enderror' placeholder='0.00'/>");$("div#budget").append(e)}}const hidden=document.getElementById("hidden-input");document.getElementById("button").onclick=(()=>hidden.click()),hidden.onchange=(e=>{var l,t,r=e.target.files,a=function(e){console.log(e),image.src=e,$modal.modal("show")};r&&r.length>0&&(t=r[0],URL?a(URL.createObjectURL(t)):FileReader&&((l=new FileReader).onload=function(e){a(l.result)},l.readAsDataURL(t)))}),$modal.on("shown.bs.modal",function(){cropper=new Cropper(image,{aspectRatio:1,viewMode:3,preview:".preview"})}).on("hidden.bs.modal",function(){cropper.destroy(),cropper=null}),$("#crop").click(function(){canvas=cropper.getCroppedCanvas({width:160,height:160}),canvas.toBlob(function(e){url=URL.createObjectURL(e);var l=new FileReader;l.readAsDataURL(e),l.onloadend=function(){var e=l.result;$("ul#gallery").prepend('<li class="float-left w-1/2 md:w-1/4 px-2 py-2 relative"><img src="" class="w-full rounded-lg"></li>'),$("ul#gallery li:first-child img").attr("src",e),$("ul#gallery li:first-child").append('<a class="delete absolute bottom-5 right-5 z-10" onclick="$(this).parent().remove()"><i class=" text-gray-100 hover:text-gray-700 far fa-trash-alt"></i></a>'),$modal.modal("hide")}})}),$("button#sendRequest").click(function(){var e=$("ul#gallery img"),l=[];if(console.log(e),e.length>0)for(let t=0;t<e.length;t++){const r=e[t];l.push(r.src)}$("textarea#images").val(JSON.stringify(l))});
