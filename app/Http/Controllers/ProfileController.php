...
class ProfileController extends Controller
{
  ...
  public function updateProfile(Request $request, $user_id)
  {
    ...
    if (isset($input['instagram_check']) && $input['instagram_check']) {
      $checked = $input['instagram_check'];
      if ($checked[0] == 'checked')
        $profile[0]->instagram_check = 1;
    } else
      $profile[0]->instagram_check = 0;
    ...
    if (isset($input['youtube_check']) && $input['youtube_check']) {
      $checked = $input['youtube_check'];
      if ($checked[0] == 'checked')
        $profile[0]->youtube_check = 1;
    } else
      $profile[0]->youtube_check = 0;
    ...
    if (isset($input['tiktok_check']) && $input['tiktok_check']) {
      $checked = $input['tiktok_check'];
      if ($checked[0] == 'checked')
        $profile[0]->tiktok_check = 1;
    } else
      $profile[0]->tiktok_check = 0;
    ...
    if (isset($input['website_check']) && $input['website_check']) {
      ...
      if ($checked[0] == 'checked')
        $profile[0]->website_check = 1;
    } else
      $profile[0]->website_check = 0;
    ...
    if (count($saved_ids) > 0) {
      foreach ($saved_ids as $user_id) {
        ...
        $profile = Profile::where('user_id', '=', $user_id->user2_id)
          ->select('instagram', 'instagram_follows', 'youtube', 'youtube_follows', 'tiktok', 'tiktok_follows', 'website')
          ->get();
